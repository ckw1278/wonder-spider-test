...
class Gmarket extends Codec {
  ...
  async productInfoFromHtml($, uri) {
    //console.log($(this.categorySelector()).eq(2).children('a').text());
    const aa = $('#txtReviewTotalCount');

    if (aa.length > 0) {
    }

    const deepLink = await this.generateDeepLink(uri);
    //for(selector of this.imageUriSelectors()) {
    //  const imgUri = $(selector).attr('src');
    //}
